...
generate-ssh-keys: ## Generates SSH keys for the JWT library
	...
	U_ID=${UID} docker exec -it --user ${UID} ${DOCKER_BE} openssl genrsa -passout pass:048bf11bfb9715e3d0e371c27de95384 -out config/jwt/private.pem -aes256 4096
	U_ID=${UID} docker exec -it --user ${UID} ${DOCKER_BE} openssl rsa -pubout -passin pass:048bf11bfb9715e3d0e371c27de95384 -in config/jwt/private.pem -out config/jwt/public.pem